...
// Lecture d'un contrat OpenAPI 3.1 et validation JSON Schema minimale (même validateur que les BFF,
// tests/support/openapi-contract.ts). Il ne couvre que les mots-clés émis par zod-to-openapi dans les
// contrats Mairie360 : tout mot-clé inconnu fait échouer la validation plutôt que d'être ignoré.
...
class OpenApiContract {
  ...
  /** Couples `METHOD /template` déclarés par le contrat. */
  operations() {
    return Object.entries(this.document.paths).flatMap(([template, item]) =>
      Object.keys(item).filter((method) => HTTP_METHODS.includes(method)).map((method) => `${method.toUpperCase()} ${template}`));
  }
